...
void RWorkstationConfigShop::applyLookups(const QJsonObject &jdoc)
{
...
void RWorkstationConfigShop::applyConfig(const QJsonObject &config)
{
    ...
    ui->leReceiptPrinterLeftRightMargins->setText(
        QString::number(qMax(0, config.value(QStringLiteral("f_precheck_margins")).toInt())));
...
QJsonObject RWorkstationConfigShop::collectConfig() const
{
    ...
    jo.insert(QStringLiteral("f_precheck_margins"),
              qMax(0, ui->leReceiptPrinterLeftRightMargins->text().trimmed().toInt()));
