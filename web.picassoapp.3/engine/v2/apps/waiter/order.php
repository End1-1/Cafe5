...
class Order extends Auth
{
    ...
    WHERE og.f_header = ? 
      AND og.f_state = 1
      AND ((og.f_isservice IS NULL OR og.f_isservice = 0) 
...
SQL;
    ...
    EOD;
        ...
        foreach ($print_data as $pd) {
            ...
            // Декодируем данные конкретно ЭТОЙ строки (сохраняем f_emarks и прочее)
            $itemData = json_decode($pd["f_data"] ?? "{}", true);

            // Собираем данные для принтера №1
            ...
            // Собираем данные для принтера №2
            ...
            // Ставим отметку о печати ТОЛЬКО для этого товара
            $itemData["f_printed"] = true;
            $itemData["f_print_time"] = date("Y-m-d H:i:s");

            // Сразу сохраняем обновленный JSON обратно в БД для конкретной строки
            $this->update("o_goods", [
