...
DlgSetWaiterOrderCL::DlgSetWaiterOrderCL(C5User *user) :
    C5Dialog(user),
...
bool DlgSetWaiterOrderCL::getCL(C5User *user, QString &code, QString &name)
{
    DlgSetWaiterOrderCL d(user);
...
void DlgSetWaiterOrderCL::on_btnNew_clicked()
{
    ...
    if(DlgNewCL::createCL(mUser, code, name)) {
        int r = ui->tbl->addEmptyRow();
