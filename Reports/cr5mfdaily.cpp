...
void CR5MfDaily::processTaskDbClick()
{
    ...
    if(!C5Selector::getValueOfColumn(mUser, cache_mf_active_task, vals, 3)) {
        return;
    }
...
void CR5MfDaily::addProcess()
{
    ...
    if(!C5Selector::getValue(mUser, query, vals)) {
        return;
    }
    ...
    db[":f_worker"] = item->data(Qt::UserRole + 1);
    db[":f_product"] = vals.at(3).toInt();
    db[":f_process"] = vals.at(5).toInt();
    db[":f_qty"] = 0;
    db[":f_price"] = vals.at(8).toDouble();
    db[":f_taskid"] = taskid;
