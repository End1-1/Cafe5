...
void DishItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (index.data(Qt::UserRole + 100).toInt() == -1) {
        painter->fillRect(option.rect, QColor::fromRgb(100, 255, 100));
        painter->drawRect(option.rect);
        QRectF tr = option.rect;
        tr.adjust(2, 2, 2, 2);
        painter->drawText(tr, index.data(Qt::DisplayRole).toString());
    } else if (index.data(Qt::UserRole + 100).toInt() == -2) {
        painter->fillRect(option.rect, QColor::fromRgb(100, 255, 255));
        painter->drawRect(option.rect);
    } else {
