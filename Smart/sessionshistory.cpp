    ...
    ui(new Ui::SessionsHistory)
{
    ...
    while (db.nextRow()) {
        ...
        ui->tbl->setItem(r, 1, new QTableWidgetItem(QString("%1 %2").arg(db.getDate("f_datein").toString(FORMAT_DATE_TO_STR), db.getTime("f_timein").toString(FORMAT_TIME_TO_STR))));
        ui->tbl->setItem(r, 2, new QTableWidgetItem(QString("%1 %2").arg(db.getDate("f_dateout").toString(FORMAT_DATE_TO_STR), db.getTime("f_timeout").toString(FORMAT_TIME_TO_STR))));
...
void SessionsHistory::on_btnPrintSession_clicked()
{
    ...
    p.br();
    p.ctext(dbuser->fullName(userid));
    p.br();
