...
class Cashbox extends Auth
{
    ...
    public function Close($params)
    {
        ...
        $sql = <<<SQL
        SELECT SUM(coalesce(JSON_VALUE(f_data, '$.f_amount_cash'),0)) AS f_amount_cash,
        SUM(coalesce(JSON_VALUE(f_data, '$.f_amount_card'),0)) AS f_amount_card,
        SUM(coalesce(JSON_VALUE(f_data, '$.f_amount_idram'),0)) AS f_amount_idram,
        SUM(coalesce(JSON_VALUE(f_data, '$.f_amount_other'), 0)) AS f_amount_other
        FROM o_header WHERE f_cash_session_id=?;
        SQL;
        $today = $this->select($sql, "i", [$cash_session["f_id"]])->fetch_assoc();
        $this->select("delete from cash_operations where f_session_id=?", "i", [$cash_session["f_id"]], true);
        ...
        foreach ($payment["types"] as $pt) {
            ...
            if (($today[$pn] ?? 0) > 0) {
                $this->insert("cash_operations", [
                    ...
                    "f_debit" => $today[$pn],
                    "f_comment" => Translator::t("Closing") . " " . Translator::t($payment["names"][$pt])
                ]);
            }
        }
        $this->commit();
