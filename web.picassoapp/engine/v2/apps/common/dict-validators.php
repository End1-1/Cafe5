...
return [
    "save" => [
        "c_partners" =>   [
            ...
            "f_info" => "nullable|string|max:255",
            ...
            "f_price_politic" => "nullable|integer",
    ...
    "select" => [
        "c_partners" => [
            "sql" => <<<EOD
            select p.f_id, p.f_category, p.f_state, p.f_group,
            d1.f_value as f_category_name,d2.f_value as f_state_name,
            p.f_taxcode, p.f_taxname, p.f_name, p.f_phone, p.f_contact, p.f_info,
            p.f_address, p.f_legal_address, p.f_email, p.f_permanent_discount,
            p.f_price_politic, p.f_manager
            from c_partners p
            left join l_dictionary d1 on d1.f_dict='c_partners_category' and d1.f_dict_id=p.f_category and d1.f_lang=?
            left join l_dictionary d2 on d2.f_dict='c_partners_state' and d2.f_dict_id=p.f_state and d2.f_lang=?
            where 1=1
            order by p.f_name
            EOD
        ]
