-- Deployed automatically by run_update.php (glob of engine/db/sp/*.sql).
-- Cross-DB: reads cafe5.a_store, writes valshin.store_*.
-- Does not execute migration; call SELECT valshin.sf_migrate_stock_a_to_store2() manually,
-- or run ../migrate_stock_a_to_store2.sql as a standalone script.
-- Note: run_update creates the function in whatever DB it connects to; prefer the
-- standalone script (USE valshin) when deploying for cross-DB migration.
