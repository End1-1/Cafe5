...
bool C5PrintReceiptThread::print(const QStringList &dbParams)
{
    ...
    p.br();
    p.setFontBold(true);
    p.ctext(__translator.tt("Receipt #") + QString("%1%2").arg(fHeaderInfo["f_prefix"].toString()).arg(fHeaderInfo["f_hallid"].toString()));
    p.br();
    ...
    if (fHeaderInfo["f_discountfactor"].toDouble() > 0.001) {
        p.ltext(QString("%1 %2%").arg(__translator.tt("Discount"), float_str(fHeaderInfo["f_discountfactor"].toDouble() * 100, 2)), 0);
