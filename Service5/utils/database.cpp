...
bool Database::open(const QString &configFile)
{
    ...
    return open(s.value("db/host").toString(), s.value("db/schema").toString(), s.value("db/username").toString(),
                s.value("db/password").toString(), s.value("db/port").toInt());
}

bool Database::open(const QString &host, const QString &schema, const QString &username, const QString &password, int port)
{
    fSqlDatabase = QSqlDatabase::addDatabase(fDatabaseDriver, fDatabaseNumber);
    fSqlDatabase.setHostName(host);

    if(port > 0) {
        fSqlDatabase.setPort(port);
    }
...
{
    return open(o["host"].toString(), o["schema"].toString(), o["username"].toString(), o["password"].toString(), o.value("port").toInt());
}
