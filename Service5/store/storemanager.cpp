...
void StoreManager::init(const QString &databaseName)
{
    ...
    db.exec("select d.f_date, s.f_store, s.f_base, s.f_goods, s.f_price, sum(s.f_qty*s.f_type) as f_qty,  sum(s.f_total*s.f_type) as f_total "
            "from a_store s  "
    ...
    while (db.next()) {
        ...
        s->addRecord(db.dateValue("f_date"), db.stringValue("f_base"), db.integerValue("f_goods"), db.doubleValue("f_price"), db.doubleValue("f_qty"));
    }
