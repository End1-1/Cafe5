...
void RequestManager::handle(const QString &session, const QString &remoteHost, const QString &r,
                            const QByteArray &indata, QByteArray &outdata, const QHash<QString, DataAddress> &dataMap, ContentType contentType)
{
    ...
    int ms = et.elapsed();
    if (DatabaseConnectionManager::nodb == false) {
        CommandLine cl;
        QString path;
