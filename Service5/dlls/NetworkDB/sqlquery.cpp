...
bool printfiscal(const QByteArray &indata, QByteArray &outdata, const QHash<QString, DataAddress> &dataMap, const ContentType &contentType)
{
    QElapsedTimer et;
    et.start();
    ...
    if (jfiscal["rseq"].toInt() == 0 && fiscal) {
        ...
        if (!db.query(QString("call sf_create_shop_tax('%1')").arg(QString(QJsonDocument(jtax).toJson(QJsonDocument::Compact))))) {
...
bool printbill(const QByteArray &indata, QByteArray &outdata, const QHash<QString, DataAddress> &dataMap, const ContentType &contentType)
{
    CommandLine cl;
    QString path;
    cl.value("dllpath", path);
    ...
    int bs = jConfig["recipe_font_size"].toInt();
        ...
        p.br();
        p.setFontBold(true);
        p.ctext(__translator.tt("Receipt #") + QString("%1%2").arg(jHeader["f_prefix"].toString()).arg(jHeader["f_hallid"].toInt()));
        p.br();
    ...
    //        p.br();
        ...
        if (jHeader["f_discountfactor"].toDouble() > 0.001) {
            p.ltext(QString("%1 %2%").arg(__translator.tt("Discount"), float_str(jHeader["f_discountfactor"].toDouble() * 100, 2)), 0);
...
//                         + preorder["f_prepaidpayx"].toDouble();
    ...
    //    }
        ...
        sql = QString("update o_header set f_print=abs(coalesce(f_print, 0))+1 where f_id='%1'").arg(jHeader["f_id"].toString());
        db.query(sql);
        return rh.setResponse(HTTP_OK, QJsonDocument(jresponse).toJson(QJsonDocument::Compact));
}
