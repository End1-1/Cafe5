...
CE5CreateTablesForHall::CE5CreateTablesForHall(C5User *user) :
    C5Dialog(user),
...
void CE5CreateTablesForHall::createTableForHall(C5User *user)
{
    CE5CreateTablesForHall *d = new CE5CreateTablesForHall(user);
    d->exec();
    delete d;
}
...
void CE5CreateTablesForHall::on_btnCreate_clicked()
{
    ...
    for(int i = ui->leStart->getInteger(); i < ui->leStart->getInteger() + ui->leCount->getInteger() ; i++) {
